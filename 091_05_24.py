...
array_modulo_E_cortical[condicion_cortical] = 84370*(array_BVTV[condicion_cortical] ** 2.58) * (array_alpha ** 2.74)*1e3
mean_array_modulo_E_cortical = 84370*(array_BVTV[condicion_cortical] ** 2.58) * (array_alpha ** 2.74)*1e3
...
for dmes in range(1, 2):
    ...
    for dday in range(1, 24):  # Ciclo externo con dtime va de 1 a 24
        ...
        for cycle in range(1000, 10001, 1000):  # Ciclo interno va de 1000 en 1000 hasta 10000
            ...
            array_rho_cortical_gr_cm_3 = array_BVTV[condicion_cortical]*(1.41 +1.29*array_alpha) 
            
            
            array_modulo_E2_cortical = 84370 * (array_BVTV[condicion_cortical] ** 2.58) * (array_alpha ** 2.74) *1e3
...
# 
#----------------------------------------------------------------------------------------------
    for dday in range(1, 13):  # Ciclo externo con dtime va de 1 a 5
        ...
        for cycle in range(1000, 10001, 1000):  # Ciclo interno va de 1000 en 1000 hasta 10000
            ...
            # ---------------------------------------------------------------------------------------------------------
            array_error= ((1/array_BVTV)**2)*array_psi - 50
            array_error_cortical = ((1/array_BVTV[condicion_cortical])**2)*array_psi - 50
            fem_function_array_error.x.array[:]= array_error
            ...
            array_rdot = np.where(array_error > width,
                      ...
                      np.where((array_error <= width) & (array_error >= 0),
                               ...
                               np.where((array_error < 0) & (array_error >= -width),
                                        ratecon2 * array_error,
                                        ratecon1 * array_error + (ratecon1 - ratecon2) * width)))
            array_rdot_cortical = np.where(array_error_cortical > width,
                      ratecon4 * array_error_cortical + (ratecon3 - ratecon4) * width,
                      np.where((array_error_cortical <= width) & (array_error_cortical >= 0),
                               ratecon3 * array_error_cortical,
                               np.where((array_error_cortical < 0) & (array_error_cortical >= -width),
                                        ratecon2 * array_error_cortical,
                                        ratecon1 * array_error_cortical + (ratecon1 - ratecon2) * width)))
            ...
            array_porosity_cortical= 1 - array_BVTV[condicion_cortical]
            array_porosity2c = array_porosity_cortical * array_porosity_cortical
            array_porosity3c = array_porosity2c * array_porosity_cortical
            array_porosity4c = array_porosity3c * array_porosity_cortical
            array_porosity5c = array_porosity4c * array_porosity_cortical
            array_Svc = 0.03226 * array_porosity_cortical - 0.09394 * array_porosity2c + 0.13396 * array_porosity3c - 0.10104 * array_porosity4c + 0.02876 * array_porosity5c

    
            array_derivative_NBMU = array_Sv * array_rdot
            array_derivative_NBMU_cortical = array_Svc*array_rdot_cortical
            #-------------------------------------------------------------------------------------------
            ...
            expression_array_dot_vr_c = array_BVTV[condicion_cortical] * array_derivative_NBMU_cortical * escalar_V_bmu * escalar_tmp_ar2 * t_prima_100
            expression_array_dot_vb_c = array_derivative_NBMU_cortical * escalar_tmp_af4 * escalar_V_bmu * t_prima_100
            ...
            escalar_vm = escalar_vm_prim*(t_prima_100 -escalar_t_nm + 12)/escalar_t_prim
            print(f"el valor de escalar_vm:{escalar_vm}")
            
            array_vm = (1/array_BVTV_previous) * escalar_vm * expression_array_dot_vb_c + (1/array_BVTV_previous) * escalar_vm * expression_array_dot_vr_c
            array_BVTV_previous = expression_array_dot_vb_c + expression_array_dot_vr_c
            escalar_promedio_vm = np.mean(array_vm)
            ...
            #-------------------------------------------------------------------------------------------
            array_alpha =(escalar_rho_mineral*array_vm/(escalar_rho_mineral*array_vm + escalar_v_org* escalar_rho_org))
            escalar_promedio_alpha = np.mean(array_alpha)
